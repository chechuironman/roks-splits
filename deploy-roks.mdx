...
There is a CLI limitation that does not allow the creation of several zones deployment for nodes from the start, it needs to be added after, either change the `default` worker pool from `default`, the `default` worker pool is deleted right after creation of the cluster. The public endpoint is disabled for security, public outbound connectivity is enabled through the public gateways though.     
...
<InlineNotification>
The flavor recomended to deploy a minimum version of Foundational Services in the Infra nodes is `bx2.8x32`
...
<InlineNotification>
Once there is another worker pool we can delete the default worker-pool
  ```
...
To create the Cloud Pak worker pool:
  ...
  <InlineNotification>
  The flavor for these ndoes will depend on the cloudpak that will be installed on them.
  </InlineNotification>
  ```
...
```
The worker nodes need to be labeled and tainted:    
